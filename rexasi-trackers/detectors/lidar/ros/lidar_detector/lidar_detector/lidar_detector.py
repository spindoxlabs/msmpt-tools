...
class DetectorNode(Node):
    def __init__(self):
        ...
        self.conf_tresh = self.get_parameter("conf_tresh").get_parameter_value().double_value
        ...
        # Call model on empty image to initialize it and have faster 
        # inference on firs scan
        self._detector(np.zeros(self.ranges))
    ...
    def _scan_callback(self, msg):
        ...
        # added ad hoc for rexasi dataset to limit detections in lab area
        lab_min_y = -4
        lab_max_y = 3
        lab_min_x = -3.3
        lab_max_x = 4

        centers = [[x, y] for x, y in centers if x > lab_min_x and x < lab_max_x and y > lab_min_y  and y < lab_max_y]
